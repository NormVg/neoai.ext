...
async function extractCodingQuestion() {
  ...
  const config = window.NEOPASS_AI_CONFIG || {
    serverUrl: 'http://localhost:3001',
    endpoints: { solveCode: '/api/solve-code' },
    timeout: 30000
  };

  const serverUrl = `${config.serverUrl}${config.endpoints.solveCode}`;

  try {
    console.log('[extractCodingQuestion] Calling local AI server:', serverUrl);
    ...
    const data = await response.json();
    console.log('[extractCodingQuestion] AI server response:', data);

    if (data.success && data.code) {
      let cleanedCode = data.code.trim()
        .replace(/^```[a-z]*\n/, '')
        .replace(/\n```$/, '');

      console.log('[AI Answer] Cleaned response length:', cleanedCode.length);
      ...
      window.dispatchEvent(new CustomEvent('NEOPASS_INSERT_CODE', {
        detail: { code: cleanedCode }
      }));

      console.log('[AI Answer] Dispatched code insertion event to page context');
    } else {
      console.error('[extractCodingQuestion] No code in response:', data);
    }
  } catch (error) {
    if (error.name === 'AbortError') {
      console.error('[extractCodingQuestion] Request timed out');
    } else {
      console.error('[extractCodingQuestion] Error:', error);
    }
  }
}
...
document.addEventListener('keydown', (event) => {
  // Use Option (Alt) on all platforms
  if (event.altKey && event.shiftKey && event.code === 'KeyH') {
    handleHackerRankMCQ();
  }
});

// Listen for code request from exam.js (Alt+Shift+T) - Uses local AI server
window.addEventListener('NEOPASS_REQUEST_CODE_TYPED', async function (event) {
  ...
  try {
    ...
    const data = await response.json();
    console.log('[content.js] AI server response:', data);
    ...
    if (data.success && data.code) {
      ...
      console.log('[content.js] Cleaned code length:', cleanedCode.length);
      ...
      console.error('[content.js] No code in response:', data);
    }
  } catch (error) {
    if (error.name === 'AbortError') {
      console.error('[content.js] Request timed out');
    } else {
      console.error('[content.js] Error calling local AI server:', error);
    }
  }
});
